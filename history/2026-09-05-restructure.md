...
- **Linear: done.** Project [Task Module redesign](https://linear.app/rentok/project/task-module-redesign-4bafc030f838) under team RentOk: stage issues REN-661 to REN-667 (the estimate goes there), 22 sub-issues REN-669 to REN-690 under stages 1 and 2, and REN-668 for the D69 practice question. 05 and the README link every row. Links point at `main`; switched after the merge on 5 Sep.
- **Committed and pushed** (Sanchay, 5 Sep): `584493f` the pure moves, `67914e7` the content, then the Linear links; then merged to `main` by fast-forward the same day (Sanchay). The checklist-library pointer is `11b8ac5` on its branch `docs/task-module-redesign-brief`. 
- **Vault mirror: done** (`RentOk/PRDs/Task Module/`, 13 notes, synced 2026-09-05 through the obsidian skill). Vault notes keep human-readable names and carry a source callout: README is "Task Module Docs"; 00 list and map are "Task Module Feature List" and "Task Module Feature Map"; 01 to 05 are Brief, Requirements, Build Sequence, Spec Stages 1 and 2, Engineering Asks; CHANGELOG is "Task Module Decisions"; the two reference docs and the two session handoffs keep their names. Relative links became wikilinks; zero unresolved. The repo wins on any difference.
